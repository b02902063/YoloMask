...
def test(data,
    ...
    for batch_i, D in enumerate(tqdm(dataloader, desc=s)):
        if not mask:
            (img, targets, paths, shapes) = D
        else:
            (img, segs, bboxes, targets, paths, shapes) = D
        ...
        with torch.no_grad():
            ...
            if mask:
                inf_out, train_out, attn = model_output[0]["test"], model_output[0]["bbox_and_cls"],\
                                           model_output[0]["attn"]
                bases = model_output[1]
            else:
                inf_out, train_out = model_output["test"], model_output["bbox_and_cls"]
            print([i.shape for i in inf_out])
            print([i[..., 0:5] for i in inf_out])
            print([i.tensor.shape for i in bboxes])
