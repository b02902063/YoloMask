...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    # m = model.module.model[detect_index] if hasattr(model, 'module') else model.model[detect_index]  # Detect()
    m = model.module.detect if hasattr(model, 'module') else model.detect  # Detect()
...
def merge_bases(rois, coeffs, location_to_inds=None):
    # merge predictions
    N = coeffs.size(0)
...
def compute_loss(p, targets, model, bases=None, bboxes=None, attn=None, segs=None):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # Objectness
            objectness = (1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(tobj.dtype)
            tobj[b, a, gj, gi] = objectness  # giou ratio
            ...
            if segs is not None:
                attns = attn[i][b, a, gj, gi]
                pred_masks = merge_bases(sbases[i], attns)
                tseg[i] = tseg[i].view(n, -1)
                mask_losses = F.binary_cross_entropy_with_logits(
                    pred_masks, tseg[i].to(dtype=torch.float32), reduction="none")
                mask_loss = mask_losses.mean(dim=-1)
                lmask += (mask_loss * objectness).mean()
    ...
    if segs is not None:
        lmask *= h['mask'] * s
        loss += lmask
        loss_collection = (lbox, lobj, lcls, lmask, loss)
    else:
        loss_collection = (lbox, lobj, lcls, loss)
...
def build_targets_with_segs(p, targets, model, segs, bboxes, bases):
    ...
    for i in range(det.nl):
        ...
        tsegs.append(s)
        sbases.append(bas)
    return tcls, tbox, tsegs, sbases, indices, anch
