...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.mask:
        pooler = ROIPooler(output_size=56,
                           scales=(0.5,),
                           sampling_ratio=1,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            if opt.mask:
                print(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'mask', 'total', 'targets',
                                             'img_size'))
            else:
        ...
        optimizer.zero_grad()
        if opt.mask:
            for i, (imgs, segs, bboxes, targets, paths, _) in pbar:  # batch ---------------------------------------------
                ...
                with amp.autocast(enabled=cuda):
                    ...
                    pred, attn = model_output[0]["bbox_and_cls"], model_output[0]["attn"]
                    bases = model_output[1]
        ...
        if rank in [-1, 0]:
            ...
            if tb_writer:
                if not opt.notest or final_epoch:
                    if opt.mask:
                        tags = ['train/giou_loss', 'train/obj_loss', 'train/cls_loss', 'train/mask_loss',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask', action='store_true', help='training segmentation')
